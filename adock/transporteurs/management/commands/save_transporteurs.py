...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            for (
                transporteur_siret
            ) in transporteurs_models.Transporteur.objects.values_list(
                "siret", flat=True
            ):
                try:
                    transporteur = transporteurs_models.Transporteur.objects.get(
                        pk=transporteur_siret
                    )
                except transporteurs_models.Transporteur.DoesNotExist:
                    self.stderr.write(
                        self.style.WARNING("Unable to get '%s'") % transporteur_siret
                    )
                ...
                try:
                    ...
                    self.stderr.write(
                        self.style.WARNING("Unable to save '%s'") % transporteur
                    )
                    self.stderr.write(self.style.ERROR("%s") % e)
                    count_error += 1
