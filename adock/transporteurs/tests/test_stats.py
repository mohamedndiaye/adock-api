...
class TransporteurStatsTestCase(TestCase):
    ...
    def test_stats(self):
        ...
        validated_date = (
            timezone.now() - datetime.timedelta(days=random.randint(1, STATS_NB_MONTHS - 1) * 31)
        ).replace(day=1)
        factories.TransporteurFactory(validated_at=validated_date)

        # One transporteur validated outside of the range of stats
        factories.TransporteurFactory(
            validated_at=timezone.now() - datetime.timedelta(days=STATS_NB_MONTHS * 31)
        )

        # 2 locked sheets
        factories.TransporteurFactory.create_batch(3, email_confirmed_at=validated_date)
