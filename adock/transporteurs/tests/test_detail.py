...
class TransporteurDetailTestCase(test.TransporteurTestCase):
    ...
    def test_get(self):
        ...
        self.assertEqual(
            transporteur_data["raison_sociale"], self.transporteur.raison_sociale
        )
        self.assertEqual(
            transporteur_data["debut_activite"], str(self.transporteur.debut_activite)
        )
        ...
        self.assertEqual(
            sorted(transporteur_data["specialities"]), ["TEMPERATURE", "URBAIN"]
        )
    ...
    def test_patch_log(self):
        ...
        transporteur = self.patch_transporteur({"telephone": PHONE}, 200)[
            "transporteur"
        ]
        ...
        # New value
        self.assertEqual(
            transporteur_log.data["working_area_departements"], ["2A", "56"]
        )
    ...
    def test_patch_phone_email(self):
        ...
        data = self.patch_transporteur({"telephone": PHONE, "email": EMAIL}, 200)
    ...
    def test_patch_partial_completeness(self):
        ...
        self.transporteur.refresh_from_db()
        self.assertEqual(
            data["transporteur"]["completeness"], self.transporteur.completeness
        )
        self.assertEqual(
    ...
    def test_patch_full_completeness(self):
        ...
        self.assertEqual(
            self.transporteur.working_area_departements, ["23", "45", "976"]
        )
    ...
    def test_patch_website(self):
        ...
        data = self.patch_transporteur({"website": "www.example.com"}, 200)
    ...
    def test_patch_unknow_payload(self):
        data = self.patch_transporteur({"foo": "42"}, 200)
        self.assertNotIn("foo", data["transporteur"])

    def test_patch_phone_required(self):
        # Accepted
        data = self.patch_transporteur({"email": self.transporteur.email}, 200)
        ...
        data = self.patch_transporteur({"email": self.transporteur.email}, 400)
        self.assertEqual(data["telephone"][0], "Ce champ est obligatoire.")

        data = self.patch_transporteur(
            {"telephone": PHONE, "email": self.transporteur.email}, 200
        )

    def test_patch_invalid_phone(self):
        data = self.patch_transporteur(
            {"telephone": "11223344556", "email": self.transporteur.email}, 400
        )
    ...
    def test_patch_unexisting_working_area_departements(self):
        data = self.patch_transporteur({"working_area_departements": ["20"]}, 400)
        self.assertEqual(
    ...
    def test_patch_invalid_working_area_departements(self):
        data = self.patch_transporteur({"working_area_departements": ["2034;454"]}, 400)
        self.assertEqual(
            data["working_area_departements"][0],
    ...
    def test_patch_format_working_area_departements(self):
        ...
        self.assertListEqual(
            self.transporteur.working_area_departements, ["01", "05", "10", "2A", "976"]
        )
    ...
    def test_completeness(self):
        ...
        self.transporteur.save()
        self.assertEqual(
            self.transporteur.completeness, models.COMPLETENESS_PERCENT_MIN
        )
