    ...
    def handle(self, *args, **options):
        ...
        with tempfile.TemporaryDirectory() as tmp_dirname:
            ...
            if len(filenames) > 1:
                self.stderr.write(
                    self.style.ERROR(
                        "The zip file '%s' contains several files (%s)."
                        % (feed.filename.name, filenames)
                    )
                )
                sys.exit(1)
            ...
            try:
                ...
                self.stderr.write(
                    self.style.ERROR(
                        "Unable to remove empty trailing line in registre '%s'."
                        % filename
                    )
                )
                sys.exit(1)
            ...
            try:
                ...
                sed_ps.kill()
                self.stderr.write(self.style.ERROR("Timeout: sed of '%s'." % filename))
                sys.exit(1)
            ...
            if stderr_data:
                self.stderr.write(
                    self.style.ERROR(
                        "Unable to 'psql' the file '%s' with 'import-registre.sql'."
                        % filename
                    )
                )
                self.stderr.write(self.style.ERROR(stderr_data))
